...
def one_vs_all(X, y, num_labels, lambda_, learning_rate=0.00001, n_iterations=500):
    m, n = X.shape
    all_theta = np.zeros((num_labels, n))
    for i in range(num_labels):
        y_i = (y == i).astype(int)
        initial_theta = np.zeros(n)
        all_theta[i] = gradient_descent(X, y_i, initial_theta, learning_rate, n_iterations, lambda_)
    return all_theta
...
def logreg_train(dataset_file):
    ...
    all_theta = one_vs_all(X, y_encoded, num_labels, lambda_, learning_rate=0.001, n_iterations=1000)
    
    weights_path = os.path.join(output_dir, "weights.csv")
    np.savetxt(weights_path, all_theta, delimiter=',')
    print("Model trained successfully. Files 'weights.csv' and 'stats.csv' successfully generated.")
